...
import { SKILL_ICON_REGISTRY, type SkillIcon } from "@/lib/skill-icons";
...
function SkillPill({ name, icon }: { name: string; icon: SkillIcon }) {
    const LucideIcon =
        icon?.kind === "lucide" ? SKILL_ICON_REGISTRY[icon.name] : null;
    ...
    return (
        <div className="flex items-center gap-2 px-3 py-2 rounded-xl bg-secondary/60 hover:bg-secondary border border-border/40 hover:border-primary/30 transition-all duration-200 cursor-default group">
            ...
            {LucideIcon && (
                <LucideIcon
                    className="w-5 h-5 flex-shrink-0 text-muted-foreground group-hover:text-primary transition-colors duration-200"
                    strokeWidth={2}
                    aria-hidden="true"
                />
            )}
